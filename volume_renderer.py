...
class VolumeFeatureRenderer(nn.Module):
    ...
    def volume_integration(self, normalized_pts, viewdirs, styles, raw, z_vals, rays_d, pts, return_eikonal=False, semantics=None):
        ...
        # print("feature shape inside", features.shape)
        # print("rgb shape inside is", rgb.shape)

        if semantics is None:
            rgb_sem = None
        else:
            rgb_sem = torch.sum(mask3d[semantics[0]:(semantics[0]+1)]*rgb[semantics[0]:(semantics[0]+1)], 0, keepdim=False)
            # print("rgb sem shape inside is", rgb_sem.shape)
        ...
        if semantics is not None:
            rgb_sem = -1 + 2 * torch.sum(weights * torch.sigmoid(rgb_sem), self.samples_dim)
        ...
        return rgb, features, sdf_out, mask, xyz, eikonal_term, mask2d, rgb_sem
    ...
    def render_rays(self, ray_batch, styles=None, return_eikonal=False, semantics=None):
        ...
        rgb, features, sdf, mask, xyz, eikonal_term, seg, rgb_sem = self.volume_integration(normalized_pts.detach(), viewdirs.detach(), styles.detach(), raw, z_vals, rays_d, pts, return_eikonal=return_eikonal, semantics=semantics)

        return rgb, features, sdf, mask, xyz, eikonal_term, seg, rgb_sem
    ...
    def render(self, focal, c2w, near, far, styles, c2w_staticcam=None, return_eikonal=False, semantics=None):
        ...
        rgb, features, sdf, mask, xyz, eikonal_term, seg, rgb_sem = self.render_rays(rays, styles=styles, return_eikonal=return_eikonal, semantics=semantics)
    ...
    def forward(self, cam_poses, focal, near, far, styles=None, return_eikonal=False, semantics=None):
        rgb, features, sdf, mask, xyz, eikonal_term, seg, rgb_sem = self.render(focal, c2w=cam_poses, near=near, far=far, styles=styles, return_eikonal=return_eikonal, semantics=semantics)
